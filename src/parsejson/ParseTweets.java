...
public class ParseTweets {
	...
	public static ParsedArticle parseArticle(String articleText){
		...
		//Add the raw words without polarity
		String[] wordsRaw = articleText.split(" ");
		HashMap<String, Integer> wordsRawMap = new HashMap<String, Integer>(); 
		for (String word : wordsRaw) {
			wordsRawMap.put(word, wordsRawMap.getOrDefault(word, 1));
		}
		parsed.words = wordsRawMap;
		
		String negativesRemoved = articleText.replaceAll("(not|isnt|dont) [a-z]+ ", "");
		...
		//Remove common words
		if (printUnparsed) System.out.println(unparsed);
		if (printParsed) System.out.println(articleText + "\n");
		
		if (printFreq) printFrequencies(frequencies);
		parsed.wordsPolarity = frequencies;
